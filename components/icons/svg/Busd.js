import * as React from "react";
const SvgBusd = (props) => (
  <svg
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 336.41 337.42"
    {...props}
  >
    <defs>
      <style>{".BUSD_svg__cls-1{fill:#f0b90b;stroke:#f0b90b}"}</style>
    </defs>
    <g id="BUSD_svg__Layer_2" data-name="Layer 2">
      <g id="BUSD_svg__Layer_1-2" data-name="Layer 1">
        <path
          d="m168.2.71 41.5 42.5-104.5 104.5-41.5-41.5ZM231.2 63.71l41.5 42.5-167.5 167.5-41.5-41.5ZM42.2 126.71l41.5 42.5-41.5 41.5-41.5-41.5ZM294.2 126.71l41.5 42.5-167.5 167.5-41.5-41.5Z"
          className="BUSD_svg__cls-1"
        />
      </g>
    </g>
  </svg>
);
export default SvgBusd;
